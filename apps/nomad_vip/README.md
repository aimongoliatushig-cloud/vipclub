...
The source is stored persistently at `/opt/nomad-vip/apps/nomad_vip` by default and bind-mounted into every Frappe process using `deploy/compose.nomad-app.yaml`. A commit-addressed release can set `NOMAD_VIP_APP_DIR` to the extracted app directory for reproducible deploys and rollback.
