...
def get_branch_sales_goal_query_conditions(user: str | None = None) -> str:
	user = user or frappe.session.user
	if _is_privileged(user):
		return ""
	if "Branch Manager" not in _roles(user):
		return "1 = 0"
	branch = get_branch_for_user(user)
	return f"`tabVIP Branch Sales Goal`.`branch` = {_quoted(branch or '')}"


def has_branch_sales_goal_permission(
	doc, user: str | None = None, permission_type: str | None = None
) -> bool:
	user = user or frappe.session.user
	if _is_privileged(user):
		return permission_type in (None, "read", "select", "report", "export")
	return bool(
		"Branch Manager" in _roles(user)
		and permission_type in (None, "read", "select", "report")
		and getattr(doc, "branch", None) == get_branch_for_user(user)
	)
