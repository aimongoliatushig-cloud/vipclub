...
def _verified_minutes(employee):
	...
	return _verified_minutes_from_rows(rows)
...
def get_workday():
	...
	verified_minutes, completed_days = _verified_minutes(profile.employee)
	...
	return {
		...
		"shift": shift,
		"attendance": {"checked_in": checked_in, "checked_out": checked_out, "open": state.open, "events": checkins},
		"availability": {**availability, "allowed_next": allowed},
		"summary": {"verified_minutes": verified_minutes, "completed_days": completed_days, "completed_services": reservations},
...
def submit_attendance_correction(attendance_date, correction_type, requested_time, reason, idempotency_key=None):
	...
	if correction_type not in ("Check-in", "Check-out"):
		frappe.throw(_("Засварын төрөл буруу байна."), frappe.ValidationError)
