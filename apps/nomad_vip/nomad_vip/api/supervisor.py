...
def get_readiness_queue(status="All", limit=50, cursor=0, work_date=None):
	actor = require_actor("Lead Entertainer", "Entertainer Supervisor", "System Manager")
	...
	rows = frappe.db.sql(
		...
		left join `tabVIP Daily Readiness Check` rc
			on rc.entertainer = p.name and rc.shift_assignment = sa.name
		where {where_clause}
		order by p.stage_name asc, p.name asc
...
def submit_readiness(
	...
	replay = _audit_replay(actor, "lead_entertainer.readiness.submit", idempotency_key)
	if replay:
		try:
	...
	existing = frappe.db.get_value(
		...
		{"entertainer": profile.name, "shift_assignment": shift_assignment},
		READINESS_FIELDS,
		as_dict=True,
	)
	if existing:
	...
	record_api_audit(
		actor=actor,
		action="lead_entertainer.readiness.submit",
