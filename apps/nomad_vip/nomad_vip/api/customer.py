...
def register_walk_in_customer(customer_name, phone):
	...
	if existing:
		return {"created": False, "branch": branch, "detail": get_customer_detail(existing, branch)}
	...
	frappe.db.commit()
	return {"created": True, "branch": branch, "detail": get_customer_detail(doc.name, branch)}
...
def get_customer_detail(customer, branch_filter=None):
	...
	if not frappe.has_permission("Customer", "read", customer) and not roles.intersection({"Reception", "Branch Manager", "Operation"}):
		frappe.throw(_("Not permitted"), frappe.PermissionError)
	is_central_operation = "Operation" in roles and "Branch Manager" not in roles
	if is_central_operation:
		from nomad_vip.integrations.finex import VIP_BRANCHES
		if branch_filter not in VIP_BRANCHES:
			frappe.throw(_("Select a valid VIP branch"), frappe.PermissionError)
	elif frappe.session.user != "Administrator" and not roles.intersection({"System Manager", "CEO"}) and roles.intersection({"Reception", "Branch Manager"}):
		from nomad_vip.services import get_branch_for_user
	...
	branch_profiles = frappe.get_all(
		...
		fields=["name", "branch", "membership_rank", "manual_rank", "rank_override_by", "rank_override_at", "is_banned", "ban_reason", "banned_by", "banned_at", "visit_count", "bill_count", "total_spend", "average_bill", "first_visit", "last_visit"],
		order_by="branch asc",
		limit_page_length=0,
	)
...
def set_customer_ban(customer, banned, reason):
	...
	branch = _reception_branch()
	if not branch:
		frappe.throw(_("Салбарын менежерийн эрх шаардлагатай."), frappe.PermissionError)
...
def set_customer_rank(customer, membership_rank):
	from nomad_vip.api.admin import RANKS, resolve_customer_rank
	from nomad_vip.services import require_any_role
	...
	if not branch:
		frappe.throw(_("Салбарын менежерийн эрх шаардлагатай."), frappe.PermissionError)
	if membership_rank not in ("Automatic", "Unassigned", *RANKS):
		frappe.throw(_("Сонгосон VIP зэрэглэл хүчин төгөлдөр биш байна."))
	profile_name = frappe.db.get_value(
		"VIP Customer Branch Profile", {"customer": customer, "branch": branch}, "name"
	)
	if not profile_name:
		frappe.throw(_("Тухайн салбар дахь хэрэглэгчийн бүртгэл олдсонгүй."))

	manual_rank = "" if membership_rank == "Automatic" else membership_rank
	frappe.db.set_value(
		"VIP Customer Branch Profile",
		profile_name,
		{
			"manual_rank": manual_rank,
			"rank_override_by": frappe.session.user,
			"rank_override_at": now_datetime(),
		},
	)
	profile = frappe.db.get_value(
		"VIP Customer Branch Profile",
		profile_name,
		["name", "manual_rank", "average_bill"],
		as_dict=True,
	)
	resolved_rank = resolve_customer_rank(branch, profile)
	frappe.db.set_value("VIP Customer Branch Profile", profile_name, "membership_rank", resolved_rank)
	frappe.db.commit()
	return get_customer_detail(customer, branch)
