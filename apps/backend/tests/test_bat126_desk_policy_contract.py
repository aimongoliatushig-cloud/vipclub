...
class TestBat126DeskPolicyContract(TestCase):
	...
	def test_workspaces_only_launch_the_role_apps(self):
		...
		self.assertEqual(
			...
			{"Branch Manager", "Entertainer", "Lead Entertainer", "Entertainer Supervisor", "Bartender", "Server"},
		)
		self.assertEqual(
	...
	def test_after_migrate_reconciles_the_managed_policy(self):
		...
		self.assertIn('"Bartender"', source)
		self.assertIn("ensure_bartender_roles()", source)
		self.assertIn('"Server"', source)
		self.assertIn("ensure_server_roles()", source)
		self.assertIn("ensure_desk_policy()", source)

	def test_bartender_is_scoped_to_the_staff_launcher(self):
		policy = (ROOT / "nomad_vip/desk_policy.py").read_text(encoding="utf-8")
		self.assertIn('"Bartender"', policy)
		self.assertIn('"Server"', policy)
		self.assertNotIn('ENTRY_ROLES = frozenset({"Bartender"', policy)
		self.assertNotIn('ENTRY_ROLES = frozenset({"Server"', policy)
