...
from nomad_vip.integrations.finex import VIP_BRANCHES, set_finex_schedule_snapshot_link
...
def get_manager_roster_candidates(status="Pending", search="", limit=50, cursor=0):
	...
	profiles = [row for row in profiles if row.lifecycle_status in (None, "", "Active", "On Leave")]
	return {
		"branch": branch,
		"status": status,
		"search": search,
...
def review_manager_roster_candidate(
	...
	note = (note or "").strip()
	linked_profile = (linked_profile or "").strip()
	...
	if decision == "Entertainer" and not linked_profile:
		frappe.throw(_("Finex бүртгэлийг баталгаатай ажилтны профайлтай холбоно уу."), frappe.ValidationError)
	if decision != "Entertainer" and linked_profile:
		frappe.throw(_("Зөвхөн бүжигчин ангилалд баталгаатай профайл холбоно."), frappe.ValidationError)
	...
	if existing_audit:
		...
		if (
			after.get("review_status") != decision
			or (after.get("review_note") or "") != note
			or (after.get("linked_profile") or "") != linked_profile
		):
			frappe.throw(
	...
	can_link_existing = doc.review_status == "Entertainer" and not doc.linked_profile and decision == "Entertainer"
	if doc.review_status != "Pending" and not can_link_existing:
		frappe.throw(_("Энэ бүртгэлийг өмнө нь шийдвэрлэсэн байна."), frappe.ValidationError)
	profile = None
	if decision == "Entertainer":
		profile = frappe.db.get_value(
			"VIP Entertainer Profile",
			linked_profile,
			["name", "branch", "active", "lifecycle_status"],
			as_dict=True,
		)
		if (
			not profile
			or profile.branch != branch
			or not profile.active
			or profile.lifecycle_status not in (None, "", "Active", "On Leave")
		):
			frappe.throw(_("Сонгосон баталгаатай ажилтны профайл энэ салбарт идэвхтэй биш байна."), frappe.ValidationError)
		already_linked = frappe.db.get_value(
			"VIP Finex Entertainer Candidate",
			{
				"linked_profile": linked_profile,
				"review_status": "Entertainer",
				"name": ["!=", doc.name],
			},
			"name",
		)
		if already_linked:
			frappe.throw(_("Энэ баталгаатай ажилтан өөр Finex бүртгэлтэй аль хэдийн холбогдсон байна."), frappe.ValidationError)
	before = {
	...
	doc.linked_profile = linked_profile or None
	doc.review_note = note
	doc.reviewed_by = actor.user
	doc.reviewed_at = now_datetime()
	doc.save(ignore_permissions=True)
	set_finex_schedule_snapshot_link(doc.name, profile if decision == "Entertainer" else None)
	after = {
		"review_status": doc.review_status,
		"review_note": doc.review_note,
		"reviewed_by": doc.reviewed_by,
