  ...
  return <div className="page leave-page">
    ...
    <div id="leave-kind-panel" className="leave-kind-panel" role="tabpanel" aria-labelledby={leaveKind === 'hourly' ? 'leave-tab-hourly' : 'leave-tab-day'}>
      ...
      <form className="leave-request-form leave-request-primary" onSubmit={submit} aria-busy={busy}>
        ...
        {!activeRequest ? <>
          <label><span>Шалтгаан</span><textarea value={reason} onChange={event => { setReason(event.target.value); setSuccess('') }} minLength={3} maxLength={300} aria-describedby="leave-reason-help" placeholder="Жишээ: Эрүүл мэндийн шалтгаантай" disabled={busy || !data || (leaveKind === 'hourly' ? remaining <= 0 || !cutoff.isOpen : !data.day_leave_types?.length)} required /><small id="leave-reason-help">Хамгийн багадаа 3 тэмдэгт бичнэ.</small></label>
