        ...
        <header className="topbar">
          ...
          <div className="topbar__status">
            ...
            <div className="profile-menu-wrap">
              ...
              {profileOpen ? <div className="profile-menu" role="menu"><div><strong>{session.displayName}</strong><span>{session.role} · {session.source === 'demo' ? 'Demo session' : 'Server session'}</span><small>{scopeLabel}</small></div><div className="profile-theme-setting"><span><strong>Харагдах горим</strong><small>Анхны тохиргоо Light байна</small></span><ThemeToggle /></div><button type="button" role="menuitem" onClick={() => { setProfileOpen(false); signOut() }}>Гарах</button></div> : null}
            </div>
